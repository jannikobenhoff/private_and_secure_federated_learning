...
def handle_client(client_id, model_federator, train_data, train_label, batch_size, size_of_batch, local_iter,
                  loss_func, optimizer, acc_func, loss_metric, active_client_number, number_clients, learning_rate,
                  progress_bar_clients, test_data, test_label):
    train_dataset, number_of_batches, reminder_size = prep_client_data(train_data=train_data,
                                                                       train_label=train_label,
                                                                       batch_size=batch_size,
                                                                       size_of_batch=size_of_batch,
                                                                       client_id=client_id)

    return local_train_loop(train_dataset, model_federator, number_of_batches, local_iter[client_id], loss_func,
                            optimizer,
                            acc_func,
                            loss_metric, batch_size, reminder_size, active_client_number, number_clients, learning_rate,
                            progress_bar_clients)
...
def prep_client_data(train_data, train_label, size_of_batch, batch_size, client_id):
    data = train_data[client_id]
    label = train_label[client_id]
    ...
    return train_dataset, number_of_batches, reminder_size
...
def federator(active_clients: np.array, learning_rate: float, model: Model, train_data: list, train_label: list,
    ...
    for num_iter in range(max_iter):
        ...
        with ThreadPoolExecutor() as executor:
            future_results = [
                executor.submit(handle_client, client_id, model_federator, train_data, train_label, batch_size,
                                size_of_batch, local_iter,
                                loss_func, optimizer, acc_func, loss_metric, active_client_number, number_clients,
                                learning_rate,
                                progress_bar_clients, test_data, test_label) for
                client_id in
                range(number_clients)]  # Add other necessary arguments
        ...
        #             loss_metric, batch_size, reminder_size, active_client_number, number_clients, learning_rate,
        #             progress_bar_clients, initial_trainable_weights, test_data, test_label)
        #
        #         client_data["client_" + str(active_client_number)] = compressed_data
        #         model_federator.set_weights(federator_weights)
