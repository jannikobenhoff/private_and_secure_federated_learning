...
class Block(tf.keras.Model):
    ...
    def __init__(self, in_channels, out_channels, expansion, strides, lambda_l2):
        super(Block, self).__init__()
        self.strides = strides
        channels = expansion * in_channels

        self.conv1 = layers.Conv2D(channels, kernel_size=1, use_bias=False, kernel_regularizer=l2(lambda_l2))
        self.bn1 = layers.BatchNormalization()
        self.conv2 = layers.Conv2D(channels, kernel_size=3, strides=strides, padding='same',
                                   groups=channels, use_bias=False, kernel_regularizer=l2(lambda_l2))
        self.bn2 = layers.BatchNormalization()
        self.conv3 = layers.Conv2D(out_channels, kernel_size=1, use_bias=False, kernel_regularizer=l2(lambda_l2))
        self.bn3 = layers.BatchNormalization()

        if strides == 1 and in_channels != out_channels:
            self.shortcut = tf.keras.Sequential([
                layers.Conv2D(out_channels, kernel_size=1, use_bias=False, kernel_regularizer=l2(lambda_l2)),
...
class MobileNetV2(tf.keras.Model):
    ...
    def __init__(self, num_classes, lambda_l2):
        ...
        self.conv2 = layers.Conv2D(1280, kernel_size=1, use_bias=False, kernel_regularizer=l2(lambda_l2))
        self.bn2 = layers.BatchNormalization()
    ...
    def _make_layers(self, in_channels, lambda_l2):
        ...
        for expansion, out_channels, num_blocks, strides in self.config:
            ...
            for s in stride:
                layer += [Block(in_channels, out_channels, expansion, s, lambda_l2)]
